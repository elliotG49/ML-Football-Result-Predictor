...
def combine_predictions_per_match(df_new, predictions, prediction_proba):
    ...
    Returns:
    - pd.DataFrame: DataFrame with one row per match, including averaged probabilities and final prediction.
    """
    # Add predictions and probabilities to the DataFrame
    df_new = df_new.copy()
    df_new['Predicted_BTTS'] = predictions  # Changed from 'Predicted_Winning_Team'
    df_new[['Prob_No_BTTS', 'Prob_BTTS']] = prediction_proba  # Adjusted for binary classes
    
    # Group by existing 'match_id' and compute average probabilities and retain relevant columns
    aggregated = df_new.groupby('match_id').agg({
        'Prob_No_BTTS': 'mean',
        'Prob_BTTS': 'mean',
        'winning_team': 'first',       # Assuming 'winning_team' represents BTTS outcome
        'team_name': 'first',          # Retain team names
        'opponent_name': 'first'
    }).reset_index()
    
    # Determine final prediction based on highest average probability
    aggregated['Final_Predicted_BTTS'] = aggregated[['Prob_No_BTTS', 'Prob_BTTS']].idxmax(axis=1)
    
    # Map probability column names to class labels
    probability_mapping = {
        'Prob_No_BTTS': 0,  # 0: No BTTS
        'Prob_BTTS': 1      # 1: BTTS
    }
    aggregated['Final_Predicted_BTTS'] = aggregated['Final_Predicted_BTTS'].map(probability_mapping)
...
def save_combined_predictions(df_combined, output_csv_path):
    ...
    """
    try:
        # Define the columns to include: relevant features + 'team_name' + 'opponent_name' + final prediction + probabilities
        columns_to_include = [
            'match_id', 'team_name', 'opponent_name', 'Final_Predicted_BTTS',
            'Prob_No_BTTS', 'Prob_BTTS',
            'winning_team'
        ]
...
def evaluate_model_performance_combined(df_combined, output_metrics_path, model_name):
    ...
    """
    try:
        true_labels = df_combined['winning_team']  # Ensure this column represents BTTS outcomes (0 or 1)
        predictions = df_combined['Final_Predicted_BTTS']
    except KeyError:
    ...
    precision_binary = precision_score(true_labels, predictions, average='binary', zero_division=0)
    recall_binary = recall_score(true_labels, predictions, average='binary', zero_division=0)
...
def evaluate_per_team_accuracy_combined(df_combined, output_per_team_accuracy_csv):
    ...
    try:
        ...
        required_columns = ['team_name', 'Final_Predicted_BTTS', 'winning_team']
        ...
        df_combined['Correct_Prediction'] = df_combined['Final_Predicted_BTTS'] == df_combined['winning_team']
...
def compute_and_plot_distributions_combined(df_combined, class_labels, class_names, metrics_dir):
    ...
    try:
        ...
        df_distributions = pd.DataFrame({
            ...
            'Predicted': df_combined['Final_Predicted_BTTS']
        })
        ...
        plot_class_distribution(
            ...
            title='Actual BTTS Class Distribution',
        ...
        plot_class_distribution(
            ...
            column='Predicted',
            class_names=class_names,
            title='Predicted BTTS Class Distribution',
...
if __name__ == "__main__":
    ...
    model_name = 'v6-direct-swig-rs42'  # Replace with your actual model name
    model_dir = f'/root/barnard/machine-learning/trained-models/premier-league'
    new_data_csv = f'/root/barnard/machine-learning/data-sets/premier-league/predictions.csv'  # Replace with your new data CSV path
    output_predictions_csv = '/root/barnard/machine-learning/data-prediction/tmp/combined_predictions.csv'  # Replace with desired output path
    output_metrics_json = '/root/barnard/machine-learning/data-prediction/tmp/evaluation_metrics.json'  # Path to save evaluation metrics
    output_per_team_accuracy_csv = '/root/barnard/machine-learning/data-prediction/tmp/per_team_accuracy.csv'  # Path to save per-team accuracy
    output_per_team_accuracy_plot = '/root/barnard/machine-learning/data-prediction/tmp/per_team_accuracy.png'  # Path to save the plot
    
    # Define class labels and names for distribution plots
    class_labels = [0, 1]  # 0: No BTTS, 1: BTTS happened
    class_names = {0: 'No BTTS', 1: 'BTTS'}
    
    # Define mapping from class labels to odds columns
    # Since odds are not used, this can be removed or left empty
    # class_to_odds_column = {
    #     0: 'odds_no_btts',  # Replace with your actual column name for No BTTS odds
    #     1: 'odds_btts'      # Replace with your actual column name for BTTS odds
    # }
    
    # Define confidence thresholds and corresponding stake amounts
    # Since betting is removed, these are no longer needed
